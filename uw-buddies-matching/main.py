...
class MatchingAlgorithm:
    ...
    def user_id_to_match_dict(self, user_id, match_basis):
        ...
        if "interests" in user["questionnaire"]:
            match["interests"] = user["questionnaire"]["interests"]

        if "social" in user:
            if "instagram" in user["social"]:
                match["instagram"] = user["social"]["instagram"]
            if "facebook" in user["social"]:
                match["facebook"] = user["social"]["facebook"]
    ...
    def apply_matches(self, matches, api_client):
        ...
        for user_id, match_list in matches.items():
            ...
            email_to_match_map = {
                match["email"]: match for match in match_list
            }

            existing_match_emails = set(
                [match["email"] for match in self.users[user_id]["matches"]])
            current_cycle_match_emails = set(
                [match["email"] for match in match_list])

            # find set difference and add to existing match list for user
            match_emails_difference = existing_match_emails.difference(
                current_cycle_match_emails)
            for match_email in match_emails_difference:
                match_list.append(email_to_match_map[match_email])

            print(f"new match list for user {user_id}: {match_list}")
